...
install_archive() {
  ...
  curl --fail --location --retry 3 --retry-connrefused --retry-delay 2 \
    --show-error --silent --output "$archive_path" "$url"
...
main() {
  ...
  install_archive shellcheck 0.11.0 shellcheck-v0.11.0.linux.x86_64.tar.gz \
